...
import { RomoBadge } from "@/components/ui";
          ...
          <div className="mx-auto max-w-4xl text-center">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {trustPoints.map((point) => (
                <span
                  key={point}
                  className="rounded-full border border-white/10 bg-white/[0.025] px-4 py-2 text-[12px] font-medium text-white/56"
                >
                  {point}
                </span>
              ))}
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-3xl text-[14px] leading-7 text-white/56">
                  For owners who want help connecting data, setting up the first Business Brain, and getting the first report ready.
                </p>
              </div>

              <div className="rounded-2xl border border-white/10 bg-black/45 px-6 py-5 text-left md:text-right">
                <p className="text-4xl font-semibold tracking-[-0.05em]">$499</p>
                <p className="mt-1 text-sm text-white/42">one-time</p>
              </div>
            </div>
          </div>
        </div>
